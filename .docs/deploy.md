...
Render port handling and configurable client-address header are implemented. The demo worker now
lives in `tools/scripts/demo-work.ts`; seeds no longer require the ignored scratchpad. The initial
...
1. A build with no `.scratchpad/`. Done for the apps and the pack on 22 September 2026. `vite.config.ts` aliased `scratchpad:*` to `.scratchpad/src/bridge/`, which 19 files under `apps/`, `engine/` and `school/` imported through the seam, and `tools/pack.ts` compiled the curriculum by running `.scratchpad/scripts/pack-work.ts`, which imported the notation engine from there, so a clean clone failed both. The decided shape resolved it: the drawings moved into `engine/parts/`, the engine into `engine/notation/`, the site's sample child onto the moved modules, and the seam, the alias and the bridge folder are gone. `npm run build` and `npm run pack` pass on a checkout without `.scratchpad/`, and `check:boundaries` refuses any scratchpad import, by alias or by path.
   ...
   Resolved during initial repository preparation: `npm run db:demo` previously spawned `.scratchpad/scripts/demo-work.ts`, so `server/__tests__/demo.itest.ts` is the one test that fails on such a checkout. It waits on the assistant and games moves, and the demo seed is not run in production. The trace of 21 September 2026 that this paragraph held listed what the seam then carried, and the moves emptied it in the order the trace gave.
...
9. The catalogue. `saveCatalogue` writes the catalogue as the owner and only the seed calls it, and the seed writes the demo family too. Decide whether production needs catalogue rows before the first family and, if so, a catalogue-only entry point run from the laptop with the owner's direct string, the way galleo runs its own owner-side scripts against production. Decision 10.
...
|---|---|---|---|
| `DATABASE_URL` | direct | `neondb_owner` | `db:migrate` in the build, the catalogue write, the seed if it is ever run there |
...
7. Optional, as galleo does before risky data work, a snapshot branch before the first real families and before any seed or data migration:
...
3. The domain. auth.md and the demo addresses use `lumischool.ai`. Confirm it, since `APP_ORIGIN`, the Resend sending domain and the From, and the passkey relying-party id when passkeys arrive all follow it and the last should be final.
...
6. Postgres major and names. Settled on 22 September 2026, in the other direction from the draft: the console made the project on Postgres 18 with Neon's defaults, database `neondb` and role `neondb_owner`, which is what galleo has, and this document now uses those names. Done, same day: `docker-compose.yml` now pins 18. The 18 image also changed its expected mount point, refusing to start against the old `.../data` volume even when empty, so the compose file mounts the named volume at `/var/lib/postgresql` instead and the old volume was dropped and recreated. `npm run db:reset` built clean against the fresh container, and `npm run check:db`, `npm run test:db` (51 tests), `npm run db:seed` and `npm run db:demo` all ran green against it.
...
10. The catalogue in production. Whether rows are needed before the first family, and the script that writes them.
...
15. The four migrations before the first commit. `0000_initial.sql` creates things that `0001_sign_in.sql` and `0002_kid_sessions.sql` then drop: `key_guess`, the three pairing functions, the trigger that cleared a child's tablets, two constraints on `keys`, and rows of kinds the product no longer has. 0002 says in its own opening lines that nothing has been deployed, and 0003 adds one column. So the history about to be committed would carry a tablet-pairing design the product abandoned, created and removed across four files, and db.md's rule that a migration which has left this machine is never edited has nothing to anchor to yet. Folding the four into one initial migration is worth doing on its own merits, and only before the first commit: the file gets shorter as well as truer, since what is dropped is never written. Done on 22 September 2026: the four are one `0000_initial.sql`, the DDL regenerated fresh from `schema.ts` with `drizzle-kit generate` against an empty baseline, and the policies, functions, triggers and grants re-merged by hand from the four originals' own text, holding their net result rather than their history, with none of tablet pairing's functions or checks. `meta/_journal.json` now holds one entry. `npm run check:db`, `npm run test:db` (51 tests, the same count as before the fold), `npm run db:reset`, `npm run db:seed` and `npm run db:demo` all ran clean against it, and `scope.test.ts` and `scope.itest.ts` confirm the policy block is still exactly what `policySql()` generates.
