...
| `engine/parts/apps/icon.ts` | `icon`, the apps' controls as one drawing with a `name`: home, journal, map, print, settings, back, sign out, add, help and sound. `ICON_LABEL` is the word each one sits beside. It moved to the root on the drawing contract (15 September 2026), with its tests in `engine/parts/apps/__tests__/`. |
